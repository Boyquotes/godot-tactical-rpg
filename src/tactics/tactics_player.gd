...
var is_joystick = false ## Is the input method a controlle?
var stage: int = 0 ## Controls the current stage in the process
#endregion
...
func configure(my_arena: TacticsArena, my_camera: TacticsCamera, my_control: TacticsControls):
	...
	ui_control.get_act("Move").connect(
			"pressed",Callable(self,"player_wants_to_move"))
	ui_control.get_act("Wait").connect(
			"pressed",Callable(self,"player_wants_to_wait"))
	ui_control.get_act("Cancel").connect(
			"pressed",Callable(self,"player_wants_to_cancel"))
	ui_control.get_act("Attack").connect(
			"pressed",Callable(self,"player_wants_to_attack"))
	ui_control.get_act("Debug_next_turn").connect(
			"pressed",Callable(self,"player_wants_to_skip_turn"))
...
func get_3d_canvas_mouse_position(collision_mask):
...
func player_wants_to_wait(): 
	curr_pawn.do_wait()
	stage = 0
func player_wants_to_skip_turn(): 
...
func _aux_select_pawn():
	var pawn = get_3d_canvas_mouse_position(2)
	var tile = get_3d_canvas_mouse_position(1) if !pawn else pawn.get_tile()
...
func select_new_location():
	var tile = get_3d_canvas_mouse_position(1)
	arena.mark_hover_tile(tile) 
